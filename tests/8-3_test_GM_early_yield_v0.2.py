...
# fn0 = wkdir + 'extdata/5_priority_assertion.csv'
# fn2 = wkdir + 'extdata/6_hesitation.csv'
fn0 = wkdir + 'extdata/7_early_yield.csv'
...
for i in range(0, data.shape[0]):
    ...
    # GM wants to try another two ways to see the results: 
    # (1) Write a for loop of 100 times that iterative over the model simulation
    # change from 
    # "oVAoBEvoAIoEAoSNvoPF" (1091 sets of the "optimial" parameter) 
    # to "oVAoBEvoAIoDAoSNvoPF" (193 sets of the "optimial" parameter), or.
    #
    # (2) Produce 100 replicates for each of the "optimial" parameter set.
    # to present different individual interactions. 
    #
    ...
    # following:
    # 
    # (1) I use his "oVAoBEvoAIoEAoSNvoPF" model 
    # (2) I use the first "optimial" parameter set to generate simulation.
    stoch_model = sc_run.get_model_with_params('oVAoBEvoAIoDAoSNvoPF')
    
    for j in range(0, stoch_model.n_parameterisations):
        sim = sc_run.run_simulation(stoch_model, initial_cp_distances=(distance_ped, distance_veh), 
        ...
        for idx_agent, i_agent in enumerate(i_plot_agents):
            ...
            other_perceived_speed = agent.other_agent.perception.states.x_perceived[1, :]
            other_perceived_distance2encounter = agent.other_agent.perception.states.x_perceived[0, :]
            fn = pyroot + '/acceleration_case' + str(i) + '-' + str(j) + '_' + str(i_agent) + '.txt'
            np.savetxt(fn, acceleration, delimiter=',')
            fn = pyroot + '/speed' + str(i) + '-' + str(j) +  '_' + str(i_agent) + '.txt'
            np.savetxt(fn, speed, delimiter=',')
            fn = pyroot + '/other_perceived_speed' + str(i) + '-' + str(j) +  '_' + str(i_agent) + '.txt'
            np.savetxt(fn, other_perceived_speed, delimiter=',')
            fn = pyroot + '/distance2encounter' + str(i) + '-' + str(j) +  '_' + str(i_agent) + '.txt'
            np.savetxt(fn, distance2encounter, delimiter=',')
            fn = pyroot + '/other_distance2encounter'  + str(i) + '-' + str(j) +  '_' + str(i_agent) + '.txt'
